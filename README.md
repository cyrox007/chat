...
Release candidate текущей ветки: `0.6.16-alpha.1` — Trust & Safety operations metrics, incident rehearsal и disabled-by-default protective holds. Последний выпущенный `main` checkpoint — `0.6.15-alpha.1`.
...
AI moderation зафиксирован как copilot: он может помогать triage, evidence summary и рекомендациями, но не является источником punitive authority. Human authority path, AI assessment, behavioral signals, reported-media workflow, operations metrics и incident rehearsal уже реализованы. Protective holds остаются default-off до production calibration; следующий отдельный Trust & Safety slice — formal privacy-retention/secure-expiry для private moderation evidence.
