...
Release candidate текущей ветки: `0.6.17-alpha.1` — bounded private moderation evidence retention и application-level secure expiry. Последний выпущенный `main` checkpoint — `0.6.16-alpha.1`.
...
AI moderation зафиксирован как copilot: он может помогать triage, evidence summary и рекомендациями, но не является источником punitive authority. Human authority path, AI assessment, behavioral signals, reported-media workflow, operations metrics, incident rehearsal и private-evidence retention/expiry уже реализованы. Protective holds остаются default-off до production calibration; следующий Trust & Safety шаг — human-reviewed threshold calibration и согласование storage backup/snapshot lifecycle.
