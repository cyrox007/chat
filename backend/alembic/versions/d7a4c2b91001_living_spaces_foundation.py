...
def upgrade() -> None:
    ...
    # Normalize comma-separated legacy tags into a queryable relation. A hashed
    # legacy slug avoids locale-dependent regex behaviour for Cyrillic/Unicode;
    # the normalized human label remains searchable and future edits replace it
    # with the regular readable Unicode slug generated by the application.
    op.execute(
        r"""
        WITH legacy_tags AS (
            SELECT
                r.uid AS room_uid,
                LEFT(regexp_replace(trim(tag.raw_tag), '\s+', ' ', 'g'), 64) AS label,
        ...
        SELECT DISTINCT
            room_uid,
            'legacy-' || md5(lower(label)) AS slug,
            label,
            created_at
        FROM legacy_tags
        WHERE label <> ''
