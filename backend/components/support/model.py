...
class GiftDefinition(Database.Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "target_scope IN ('persona', 'space', 'both')",
            name="ck_gift_definition_target_scope",
        ),
    )


class SupportLedgerEntry(Database.Base):
    """Append-only social support history.

    Optional target foreign keys are historical hints only. Snapshot fields preserve
    the meaning of an entry if the original Persona or Space is later removed.
    """

    __tablename__ = "support_ledger_entries"

    uid = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    sender_account_uid = Column(
        UUID(as_uuid=True),
        ForeignKey("accounts.uid", ondelete="SET NULL"),
        nullable=True,
    )
    target_kind = Column(String(16), nullable=False)
    target_persona_uid = Column(
        UUID(as_uuid=True),
        ForeignKey("personas.uid", ondelete="SET NULL"),
        nullable=True,
    )
    target_room_uid = Column(
        UUID(as_uuid=True),
    ...
    gift_code = Column(
        String(32),
        ForeignKey("gift_definitions.code", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "target_kind IN ('persona', 'space')",
            name="ck_support_ledger_target_kind",
        ),
...
class CosmeticEntitlement(Database.Base):
    ...
    uid = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    source_ledger_uid = Column(
        UUID(as_uuid=True),
        ForeignKey("support_ledger_entries.uid", ondelete="RESTRICT"),
        nullable=False,
        unique=True,
    )
    persona_uid = Column(
    ...
    __table_args__ = (
        CheckConstraint(
            "(persona_uid IS NOT NULL AND room_uid IS NULL) OR "
            "(persona_uid IS NULL AND room_uid IS NOT NULL)",
