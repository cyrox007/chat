            ...
            try:
                ...
                async with Database.sessionmaker() as db:
                    revoked = await revoke_platform_restriction(
                        db,
                        admin_uid,
                        uuid_from_string(created_restriction_uids[1]),
                ...
                async with Database.sessionmaker() as cleanup_db:
                    ...
                    await cleanup_db.execute(delete(AccountRole).where(AccountRole.account_uid.in_(account_uids)))
...
def uuid_from_string(value: str):
