...
@get_session
async def refresh_tokens(request: Request, response: Response, db_session = None):
    ...
    if not refresh_token:
        ...
        return {
            "status": "error",
            "message": "Refresh token отсутствует в куках."
        }
    ...
    try:
        ...
        if not user_uid:
            ...
            return {
                "status": "error",
                "message": "Не удалось получить user_uid из refresh_token."
            }
    except PyJWTError:
        ...
        return {
            "status": "error",
            "message": "Ошибка валидации refresh_token."
        }
    ...
    try:
        ...
        if existing_record:
            ...
            response.status_code = status.HTTP_403_FORBIDDEN
            return {"status": "bed", "message": "Invalid or inactive refresh token"}
