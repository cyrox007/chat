...
- Current release: **0.6.5-alpha.2**.
- Current milestone: **0.6.x-alpha** — Pre-beta hardening continues.
- Proven so far: PostgreSQL 16 migration/recovery; Redis 7.2 distributed semantics and restart recovery; real multi-process Uvicorn/WebSocket delivery; rolling-restart reconnect with fresh one-time tickets; bounded per-socket outbound queues with slow-consumer isolation; persistent systemd-owned production listener, health-gated rolling deploy and staged SPA publish.
